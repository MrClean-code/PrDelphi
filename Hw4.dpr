...
var k:integer;
begin
  writeln('Введите приказ: 1 - up, 2 - left, 3 - right, 4 - back');
  readln(k);
  case k of
    1: make_PR:=up;
    2: make_PR:=left;
...
var k:integer;
begin
  writeln('Введите курс: 1 - north, 2 - south, 3 - west, 4 - east');
  readln(k);
  case k of
    1: make_CR:=north;
    2: make_CR:=south;
...
begin
  writeln('Начальный приказ: ');
  PR:= make_PR;
  writeln('Начальный курс: ');
  K1:= make_CR;
  PR2:=def_order_course(PR);
  writeln('Измененный курс: ');
  writeln(names[PR2]);
